...
static int
gen_eval_operands (or1ksim *sim,FILE *fo, int insn_index, int level)
{
  struct insn_op_struct *opd = sim->op_start[insn_index];
  int i;
  int num_ops;
  int nbits = 0;
  int set_param = 0;
...
*/
    ...
    if ((opd->type & OPTYPE_DIS) && (opd->type & OPTYPE_OP)) {
      ...
      if (opd->type & OPTYPE_SIG)
        shift_fprintf (level, fo, "if(%c & 0x%08x) %c |= 0x%x;\n",
                       ...
                       0xffffffff << sbit);
      opd++;
      shift_fprintf (level, fo, "*(orreg_t *)&%c += (orreg_t)sim->cpu_state.reg[(insn >> %i) & 0x%x];\n",
                     'a' + num_ops, opd->type & OPTYPE_SHR,
                     (1 << opd->data) - 1);
      dis = 1;
      dis_op = num_ops;
    ...
    if (opd->type & OPTYPE_OP) {
      ...
      if ((opd->type & OPTYPE_REG) && !dis) {
        if(!i) {
          shift_fprintf (level, fo, "#define SET_PARAM0(val) sim->cpu_state.reg[a] = val\n");
          set_param = 1;
        }
        shift_fprintf (level, fo, "#define PARAM%i sim->cpu_state.reg[%c]\n", num_ops,
                      'a' + num_ops);
...
static int output_call (or1ksim *sim, FILE *fo, int index, int level)
{
  int dis_op = -1;
  ...
  if (index >= 0)
    dis_op = gen_eval_operands (sim, fo, index, level);

  if (index < 0) output_function (fo, "l_invalid", level);

  fprintf (fo, "\n");

  shift_fprintf (level++, fo, "if (sim->do_stats) {\n");

  if (dis_op >= 0)
    shift_fprintf (level, fo, "sim->cpu_state.insn_ea = %c;\n", 'a' + dis_op);

  shift_fprintf (level, fo, "current->insn_index = %i;   /* \"%s\" */\n", index,
                 insn_name (index));

  shift_fprintf (level, fo, "analysis(sim,current);\n");
  shift_fprintf (--level, fo, "}\n");

  if (write_to_reg)
    shift_fprintf (level, fo, "sim->cpu_state.reg[0] = 0; /* Repair in case we changed it */\n");
  shift_fprintf (--level, fo, "}\n");
  return 0;
}
...
static int generate_header (FILE *fo)
{
  ...
  fprintf (fo, "#include \"siminstance.h\"\n\n" );
  fprintf (fo, "static void decode_execute (or1ksim* sim,struct iqueue_entry *current)\n{\n");
...
static int generate_body (or1ksim *sim, FILE *fo, unsigned long *a, unsigned long cur_mask, int level)
{
  ...
  if (!(*a & LEAF_FLAG)) {
    ...
    for (i = 0; i <= mask; i++, a++) {
      if (!*a) {
        ...
        shift_fprintf (level, fo, "case 0x%x:\n", i);
        generate_body (sim, fo, sim->automata + *a, cur_mask | (mask << shift), ++level);
        shift_fprintf (level--, fo, "break;\n");
        prev_inv = 0;
      }
    }
...
int main (int argc, char *argv[])
{
  ...
  /* THIS SUCKS: It now is an amputated simulator, but it should work */
  ...
  if (generate_body (sim, fo, sim->automata, 0, 1)) {
