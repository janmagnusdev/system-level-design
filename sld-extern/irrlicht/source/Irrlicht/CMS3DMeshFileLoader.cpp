...
bool CMS3DMeshFileLoader::load(io::IReadFile* file)
{
	...
	// find file size
	const unsigned long fileSize = file->getSize();
	os::Printer::log("File size: ", core::stringc(unsigned(fileSize)).c_str());

	// read whole file

	u8* buffer = new u8[fileSize+1024];
	...
	os::Printer::log("ptr: ", core::stringc((unsigned)(intptr_t)pPtr).c_str());
